...
async function testKomatsuAnalysis() {
	...
	try {
		...
		// コマツのデータを取得して分析
		const result: StockAnalysisResult = await TechnicalAnalyzer.analyzeStock(
			"6301.T",
			"1y",
		);
...
function displayAnalysisResult(result: StockAnalysisResult) {
	...
	console.log(
		`更新日時: ${new Date(result.lastUpdated).toLocaleString("ja-JP")}\n`,
	);
	...
	console.log(
		`前日比: ${result.priceData.change >= 0 ? "+" : ""}¥${result.priceData.change} (${result.priceData.changePercent >= 0 ? "+" : ""}${result.priceData.changePercent}%)\n`,
	);
	...
	console.log(
		`  25日MA: ¥${!isNaN(movingAverages.ma25) ? movingAverages.ma25.toLocaleString() : "N/A"}`,
	);
	console.log(
		`  50日MA: ¥${!isNaN(movingAverages.ma50) ? movingAverages.ma50.toLocaleString() : "N/A"}`,
	);
	console.log(
		`  200日MA: ¥${!isNaN(movingAverages.ma200) ? movingAverages.ma200.toLocaleString() : "N/A"}`,
	);
	...
	console.log(
		`  Signal: ${!isNaN(macd.signal) ? macd.signal.toFixed(3) : "N/A"}`,
	);
	console.log(
		`  Histogram: ${!isNaN(macd.histogram) ? macd.histogram.toFixed(3) : "N/A"}`,
	);
	...
	console.log(
		`モメンタム: ${getJapaneseSignal("momentum", result.signals.momentum)}`,
	);
	console.log(
		`強度: ${getJapaneseSignal("strength", result.signals.strength)}\n`,
	);
...
async function testIndividualIndicators() {
	...
	try {
		// テスト用のダミーデータ
		const testPrices = [
			100, 102, 101, 105, 107, 106, 110, 108, 112, 115, 113, 118, 120, 119, 122,
		];
		...
		console.log("\n📈 移動平均線テスト:");
		try {
			const { MovingAverageCalculator } = await import(
		...
		console.log("\n⚡ MACDテスト:");
		try {
			const { MACDCalculator } = await import(
				"../src/lib/technical-indicators"
			);
...
async function testErrorCases() {
	...
	console.log("\n2. 空のデータ配列のテスト:");
	try {
		const { TechnicalAnalyzer } = await import(
			"../src/lib/technical-indicators"
		);
